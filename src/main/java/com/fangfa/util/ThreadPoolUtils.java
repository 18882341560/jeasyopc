...
public final class ThreadPoolUtils {
    ...
    *@return
    */
    public ThreadPoolUtils(int corePoolSize,//核心池的大小 Runtime.getRuntime().availableProcessors()
                           int maximumPoolSize, //可通过Runtime.getRuntime().availableProcessors()*2来获取
                           long keepAliveTime,//表示线程没有任务执行时最多保持多久时间会终止
                           TimeUnit unit,//参数keepAliveTime的时间单位
                           BlockingQueue<Runnable> workQueue,//一个阻塞队列，用来存储等待执行的任务,3种选择 ArrayBlockingQueue,LinkedBlockingQueue,SynchronousQueue
                           ThreadFactory threadFactory,      //线程工厂，主要用来创建线程
                           RejectedExecutionHandler handler)//表示当拒绝处理任务时的策略，也可以自定义拒接策略，自定义最好
    {
        ...
        exec = new ThreadPoolExecutor(corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue, threadFactory, handler);
    }
